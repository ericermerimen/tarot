...
import { getCardName, getCardBreed, getKeywords, getMeaning, getReflectionQuestions, getAffirmation } from '@/utils/localeCards';
...
export default function DailyCard() {
  ...
  const breed = getCardBreed(card, locale);
  const keywords = getKeywords(card, locale);
  const reflections = getReflectionQuestions(card, locale);
  const affirmation = getAffirmation(card, locale);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        ...
        <Box sx={{ mb: 3, pb: 2, borderBottom: '1px solid', borderBottomColor: 'divider' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: textFaint, letterSpacing: '0.06em' }}>
              {formatDate(new Date())}
            </Typography>
          </Box>
        </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 3 }}>
          {!isFlipped && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} style={{ textAlign: 'center', marginBottom: '16px' }}>
              <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', letterSpacing: '0.1em', color: textFaint, mb: 0.5 }}>
                {t('tapToReveal')}
              </Typography>
              <Typography sx={{ fontFamily: 'var(--font-noto-sans-tc)', fontSize: '0.75rem', color: textDim }}>
                {t('tapToRevealSub')}
              </Typography>
            </motion.div>
          )}
          <TarotCard card={card} isReversed={isReversed} isFlipped={isFlipped} onClick={handleCardClick} size="large" />
        </Box>
        ...
        <AnimatePresence>
          {showMeaning && (
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.5 }}>
              {/* Card identity */}
              <Box sx={{ mb: 3, pb: 2, borderBottom: '1px solid', borderBottomColor: 'divider' }}>
                <Typography variant="h5" sx={{ fontFamily: 'var(--font-display)', color: 'text.primary', fontWeight: 300, fontSize: { xs: '1.4rem', sm: '1.75rem' }, mb: 0.25 }}>
                  {cardName}
              ...
              <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                {card.element && (
                  <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'primary.main', letterSpacing: '0.08em' }}>
                    {t('element')} · {card.element.toUpperCase()}
                  </Typography>
                )}
                {card.zodiac && (
                  <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'primary.main', letterSpacing: '0.08em' }}>
                    {t('zodiac')} · {card.zodiac.toUpperCase()}
                  </Typography>
                )}
                {card.numerology !== undefined && (
                  <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'primary.main', letterSpacing: '0.08em' }}>
                    {t('num')} · {card.numerology}
                  </Typography>
                )}
              </Box>
              ...
              {/* Keywords */}
              <Box sx={{ mb: 3 }}>
                <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: textDim, letterSpacing: '0.1em', mb: 1 }}>
                  {t('keywords')} ————————
                </Typography>
              ...
              {lm.advice && (
                <Box sx={{ mb: 3, pl: 2, borderLeft: '2px solid', borderLeftColor: 'divider' }}>
                  <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'primary.main', letterSpacing: '0.08em', mb: 0.75 }}>
                    {t('advice')}
                  </Typography>
              ...
              <Box sx={{ mb: 3, pt: 2, borderTop: '1px solid', borderTopColor: rowBorder }}>
                <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'secondary.dark', letterSpacing: '0.1em', mb: 1 }}>
                  {t('love')}
                </Typography>
                <Typography variant="body1" sx={{ lineHeight: 1.8, color: 'text.primary' }}>
                  {lm.love}
              ...
              {reflections && (
                <Box sx={{ mb: 3, p: 2, border: '1px solid', borderColor: rowBorder }}>
                  <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: textDim, letterSpacing: '0.1em', mb: 1.5 }}>
                    {t('reflection')} ————————
                  </Typography>
                  {reflections.map((q, i) => (
              ...
              {/* Affirmation */}
              {affirmation && (
                <Box sx={{ mb: 3, p: 2, border: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', textAlign: 'center' }}>
                  <Typography sx={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'secondary.dark', letterSpacing: '0.1em', mb: 1 }}>
                    {t('affirmation')}
                  </Typography>
                  <Typography variant="body1" sx={{ color: 'text.primary', fontStyle: 'italic', lineHeight: 1.8 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                <Button variant="outlined" onClick={generateDailyCard} size="large" sx={{ flex: { xs: '1 1 100%', sm: '0 1 auto' } }}>
                  {t('drawNew')}
                </Button>
              </Box>
            </motion.div>
